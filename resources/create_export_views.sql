CREATE OR REPLACE VIEW `patentsview_exports_granted`.`applicant_not_disambiguated` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`application` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`assignee_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
...
from ((`patent`.`rawassignee` `a` left join `patent`.`assignee` `b` on(`a`.`assignee_id` = `b`.`id`)) left join `patent`.`rawlocation` `c` on(`a`.`rawlocation_id` = `c`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_granted`.`assignee_not_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
`a`.`sequence` AS `assignee_sequence`,
`a`.`assignee_id` AS `assignee_id`,
...
from (`patent`.`patent_lawyer` `a` join `patent`.`lawyer` `b` on(`a`.`lawyer_id` = `b`.`id`)) 
where `b`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_granted`.`attorney_not_disambiguated` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`figures` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`foreign_priority` AS 
select `a`.`patent_id` AS `patent_id`,
...
from (`patent`.`patent_govintorg` `a` left join `patent`.`government_organization` `b` on(`a`.`organization_id` = `b`.`organization_id`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_granted`.`inventor_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
`a`.`sequence` AS `inventor_sequence`,
`a`.`inventor_id` AS `inventor_id`,
`b`.`name_first` AS `inventor_name_first`,
`b`.`name_last` AS `inventor_name_last`,
`b`.`male_flag` AS `male_flag`,
`b`.`attribution_status` AS `attribution_status`,
`a`.`location_id` AS `location_id` 
from (`patent`.`patent_inventor` `a` left join `patent`.`inventor` `b` on(`a`.`inventor_id` = `b`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_granted`.`inventor_not_disambiguated` AS 
...
`patent`.`rawinventor`.`name_first` AS `inventor_name_first`,
`patent`.`rawinventor`.`name_last` AS `inventor_name_last`,
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`location_disambiguated` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`location_not_disambiguated` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`patent` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`rel_app_text` AS 
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`us_citation` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE VIEW `patentsview_exports_granted`.`us_rel_doc` AS 
...
from ((`patent`.`uspc` `a` join `patent`.`mainclass_current` `b` on(`a`.`mainclass_id` = `b`.`id`)) join `patent`.`subclass_current` `c` on(`a`.`subclass_id` = `c`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';
...
from (`patent`.`wipo` `a` join `patent`.`wipo_field` `b` on(`a`.`field_id` = `b`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`applicant_not_disambiguated` AS 
...
select `b`.`document_number` AS `document_number`,
...
from (`pregrant_publications`.`publication` `a` join `pregrant_publications`.`application` `b` on(`a`.`document_number` = `b`.`document_number`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`assignee_disambiguated` AS 
select `a`.`document_number` AS `document_number`,
`a`.`sequence` AS `assignee_sequence`,
`a`.`assignee_id` AS `assignee_id`,
`b`.`name_first` AS `assignee_individual_name_first`,
...
`b`.`type` AS `assignee_type`,
`c`.`location_id` AS `location_id` 
from ((`pregrant_publications`.`rawassignee` `a` left join `patent`.`assignee` `b` on(`a`.`assignee_id` = `b`.`id`)) left join `pregrant_publications`.`rawlocation` `c` on(`a`.`rawlocation_id` = `c`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`assignee_not_disambiguated` AS 
select `a`.`document_number` AS `document_number`,
`a`.`sequence` AS `assignee_sequence`,
`a`.`assignee_id` AS `assignee_id`,
`a`.`name_first` AS `assignee_individual_name_first`,
...
select `pregrant_publications`.`cpc`.`document_number` AS `document_number`,
...
select `pregrant_publications`.`cpc_current`.`document_number` AS `document_number`,
...
CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`foreign_priority` AS 
select `a`.`document_number` AS `document_number`,
...
CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`inventor_disambiguated` AS 
select `a`.`document_number` AS `document_number`,
`a`.`sequence` AS `inventor_sequence`,
`a`.`inventor_id` AS `inventor_id`,
`b`.`name_first` AS `inventor_name_first`,
...
`b`.`attribution_status` AS `attribution_status`,
`a`.`location_id` AS `location_id` 
from (`pregrant_publications`.`publication_inventor` `a` left join `patent`.`inventor` `b` on(`a`.`inventor_id` = `b`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`inventor_not_disambiguated` AS 
select `a`.`document_number` AS `document_number`,
`a`.`sequence` AS `inventor_sequence`,
`a`.`inventor_id` AS `inventor_id`,
`a`.`name_first` AS `inventor_name_first`,
`a`.`name_last` AS `inventor_name_last`,
...
select `pregrant_publications`.`ipcr`.`document_number` AS `document_number`,
...
CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`location_not_disambiguated` AS 
select `a`.`id` AS `rawlocation_id`,
`a`.`location_id` AS `location_id`,
`a`.`city` AS `city`,
`a`.`state` AS `state`,
...
CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`rel_app_text` AS 
select `pregrant_publications`.`rel_app_text`.`document_number` AS `document_number`,
...
CREATE OR REPLACE VIEW `patentsview_exports_pregrant`.`us_rel_doc` AS 
select `pregrant_publications`.`usreldoc`.`document_number` AS `document_number`,
...
`a`.`subclass_id` AS `uspc_subclass_id`,
`c`.`title` AS `uspc_subclass_title` 
from ((`pregrant_publications`.`uspc` `a` join `patent`.`mainclass_current` `b` on(`a`.`mainclass_id` = `b`.`id`)) join `patent`.`subclass_current` `c` on(`a`.`subclass_id` = `c`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';
...
`b`.`sector_title` AS `wipo_sector_title`,
`b`.`field_title` AS `wipo_field_title` 
from (`pregrant_publications`.`wipo` `a` join `patent`.`wipo_field` `b` on(`a`.`field_id` = `b`.`id`)) 
where `a`.`version_indicator` <= '{{datestring}}';
