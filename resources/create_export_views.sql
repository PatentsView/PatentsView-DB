CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_applicant_not_disambiguated` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_application` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_assignee_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_assignee_not_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
`a`.`sequence` AS `assignee_sequence`,
`a`.`assignee_id` AS `assignee_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_attorney_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_attorney_not_disambiguated` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_botanic` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_cpc_current` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_cpc_title` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_examiner_not_disambiguated` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_figures` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_foreign_priority` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_gov_interest` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_gov_interest_contracts` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_gov_interest_org` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_inventor_disambiguated` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_inventor_not_disambiguated` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_ipc_at_issue` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_location_disambiguated` AS 
...
where `location`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_location_not_disambiguated` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_other_reference` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_patent` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_pct_data` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_persistent_assignee` AS 
select `patent`.`persistent_assignee_disambig`.`current_rawassignee_id` AS `rawassignee_uuid`,
...
`patent`.`persistent_assignee_disambig`.`disamb_assignee_id_20210708` AS `disamb_assignee_id_20210708`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_persistent_inventor` AS 
select `patent`.`persistent_inventor_disambig`.`current_rawinventor_id` AS `rawinventor_uuid`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_rel_app_text` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_us_application_citation` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_us_citation` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_us_patent_citation` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_us_rel_doc` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_us_term_of_grant` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_uspc_at_issue` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_granted`.`g_wipo_technology` AS 
select `a`.`patent_id` AS `patent_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_applicant_not_disambiguated` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_assignee_disambiguated` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`sequence` AS `assignee_sequence`,
`a`.`assignee_id` AS `assignee_id`,
`b`.`name_first` AS `disambig_assignee_individual_name_first`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_assignee_not_disambiguated` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`sequence` AS `assignee_sequence`,
`a`.`assignee_id` AS `assignee_id`,
`a`.`name_first` AS `raw_assignee_individual_name_first`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_cpc_at_issue` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_cpc_current` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_foreign_priority` AS 
select `a`.`document_number` AS `pgpub_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_gov_interest` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_gov_interest_contracts` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_gov_interest_org` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_granted_patent_crosswalk` AS 
...
where `pregrant_publications`.`granted_patent_crosswalk`.`version_indicator` <= '{{datestring}}';

CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_inventor_disambiguated` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`sequence` AS `inventor_sequence`,
`a`.`inventor_id` AS `inventor_id`,
`b`.`name_first` AS `disambig_inventor_name_first`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_inventor_not_disambiguated` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`sequence` AS `inventor_sequence`,
`a`.`inventor_id` AS `inventor_id`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_ipc_at_issue` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_location_not_disambiguated` AS 
select `a`.`id` AS `rawlocation_id`,
`a`.`location_id` AS `location_id`,
`a`.`city` AS `raw_city`,
`a`.`state` AS `raw_state`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_pct_data` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`date` AS `published_or_filed_date`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_published_application` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_rel_app_text` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_us_rel_doc` AS 
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_uspc_at_issue` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`sequence` AS `uspc_sequence`,
`a`.`mainclass_id` AS `uspc_mainclass_id`,
`b`.`title` AS `uspc_mainclass_title`,
...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW `patentsview_exports_pregrant`.`pg_wipo_technology` AS 
select `a`.`document_number` AS `pgpub_id`,
`a`.`sequence` AS `wipo_field_sequence`,
`a`.`field_id` AS `wipo_field_id`,
`b`.`sector_title` AS `wipo_sector_title`,
