...
def old_match(org, govt_acc_dict, existing_lookup):
    solid_matches = []
    possible_matches = []
...
def perform_lookups(existing_lookup, govt_acc_dict, organizations, manual_inputs):
    all_solid = []
    all_possible = []
    edge_cases = {'org': [], 'with_space': [], 'wo_space': []}
    for org in organizations:  # kinda slow, ~7 minutes for 6 months of data
        ...
        solid_for_org_old, possible_for_org_old = old_match(org, govt_acc_dict, existing_lookup)
        edge_cases['org'].append(org)
        edge_cases['with_space'].append(solid_for_org)
