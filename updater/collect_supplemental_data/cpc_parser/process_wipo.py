...
def get_ipc_tech_code_field_map(ipc_tech_file):
    ...
    cleaned_ipc_tech_data = ipc_technology_data.assign(
        IPC_Code_stripped=ipc_technology_data.IPC_code.str.replace("%", ""))
...
def wipo_chunk_processor(cpc_current_data, ipc_tech_field_map, cpc_ipc_concordance_map, config):
    ...
    wipo_data = extract_wipo_data(cpc_current_data, cpc_ipc_concordance_map, ipc_tech_field_map, config)
...
def process_and_upload_wipo(config):
    ...
    cpc_query_template = "SELECT * from cpc_current c join ({base_query_template} p on p.id = c.id)"
    while True:
        start = time.
        import os
