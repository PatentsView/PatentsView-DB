...
def update_rawlocation(update_config, database='RAW_DB', uuid_field = 'id'):
    ...
    update_statement = "UPDATE rawlocation rl left join {granted_db}.location_disambiguation_mapping ldm on ldm.uuid = rl.{uuid_field} set " \
                       " rl.location_id = ldm.location_id".format(uuid_field=uuid_field, granted_db=config['PATENTSVIEW_DATABASES']['RAW_DB'])
...
def location_reduce(location_data, update_config, engine):
    ...
    location_data = location_data.groupby(['location_id', 'city', 'state', 'country']).size().reset_index().groupby(['location_id', 'city', 'state', 'country'])[[0]].max()
    location_data = location_data.reset_index()
    location_data = location_data.rename(columns={0: "location_count"})
    ...
    max_df[max_df.duplicated('location_id', keep=False)].to_sql('max_location_counts', con=engine, if_exists='append', index=False)
    
    date_df = create_date_table(engine)
    
    location_date_df = date_df.sort_values(by=['location_id','date'], ascending=False).drop_duplicates('location_id').sort_index()
    
    final_loc = one_max.append(location_date_df)
    final_loc = final_loc[['location_id', 'city', 'state', 'country']]
    final_loc = final_loc.rename(columns={'location_id':'id'})
...
def build_location_query(city, state, country):
    final_q = None
    ...
    if city is None and state is None and country is None:
        ...
        if state is not None and state in state_dict:
            if added:
                final_q = final_q +" and " + state_q.format(state=state_dict[state])
            else:
                final_q = full_q.format(score_select=state_dict[state]) + state_q.format(state=state_dict[state])
                added = True
        if country is not None and country in country_dict:
            if added:
                final_q = final_q + " and " + country_q.format(country=country)
            else:
                full_q = full_q.format(score_select=country_dict[country]) + country_q.format(country=country)
                added = True
    return final_q
...
def run_query(row):
    ...
    query = build_location_query(city, state, country)
    if query is not None:
        ...
        if len(results) > 0:
            ...
            result_dict = {'found_name':found_name, 'found_state':found_state, 'found_country':found_country, 'lat':lat, 'lon':lon, 'query':query}
        else:
            result_dict = {'found_name':None, 'found_state':None, 'found_country':None, 'lat':None, 'long':None, 'query':query}
    else:
            result_dict = {'found_name':None, 'found_state':None, 'found_country':None, 'lat':None, 'lon':None, 'query':query}
