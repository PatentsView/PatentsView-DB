...
def begin_text_merging(config):
    ...
    text_table_config = {'brf_sum_text': {
        "insert": "INSERT INTO {text_db}.brf_sum_text_{year}(uuid, patent_id, text,filename, version_indicator) SELECT id, patent_id, text, filename,'{database_version}' from {temp_db}.temp_brf_sum_text".format(
        ...
        'draw_desc_text': {
            ...
            "insert": "INSERT INTO {text_db}.detail_desc_text_{year}(uuid, patent_id, text,length, filename, version_indicator) SELECT id, patent_id, text,char_length(text), filename, '{database_version}' from {temp_db}.temp_detail_desc_text".format(
                text_db=config['DATABASE']['TEXT_DATABASE'], temp_db=config['DATABASE']['TEMP_UPLOAD_DB'],
