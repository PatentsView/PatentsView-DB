...
def begin_text_merging(config):
    ...
    text_table_config = {'brf_sum_text': {
        ...
        'draw_desc_text': {
            "insert": "INSERT INTO {text_db}.draw_desc_text_{year}(uuid, patent_id, text, sequence, version_indicator) SELECT id, patent_id, text, sequence, '{database_version}' from {temp_db}.draw_desc_text".format(
                text_db=config['DATABASE']['TEXT_DATABASE'], temp_db=config['DATABASE']['TEMP_UPLOAD_DB'],
