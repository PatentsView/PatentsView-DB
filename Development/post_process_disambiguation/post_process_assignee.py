...
def upload_rawassignee(db_con, disambiguated_folder, db):
    db_con.execute('drop table if exists rawassignee_inprogress')
    ...
    raw_assignees = pd.read_csv('{}/rawassignee_updated.csv'.format(disambiguated_folder), encoding='utf-8', delimiter='\t', keep_default_na=False, na_values='')
...
def upload_assignee(db_con, disambiguated_to_write, type_lookup, assignee_id_set):
    ...
    for assignee_id, assignee_info in disambiguated_to_write.items():
        if assignee_id in type_lookup.keys():
