...
def upload_uspc_current(db_con, uspc_current_loc):
    uspc_current_files = [f for file in os.listdir(uspc_current_loc) if f.startswith('out')]
...
def find_missed_patents(db_con, patent_set, uspc_output):
    patents_in_uspc = db_con.execute('select patent_id from {}.uspc_current'.format(new_db))
    current_exists = set()
    ...
    if len(missed_patents) > 0:
        ...
        for patent in missed_patents:
            db_con.execute('select * from uspc where patent_id ="'+str(patent)+'"')
            ...
            for d in datum:
                ...
                to_add_uspc_current.append(d['uuid'], d['patent_id'], d['mainclass_id'], d['subclass_id'], d['sequence']) 
...
if __name__ == '__main__':
    ...
    find_missed_patents(db_con, patent_set, '{}/uspc_current.csv'.format(folder))
