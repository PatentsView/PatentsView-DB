...
def check_latest_date(new_database, latest_expected_date):
	date_dict = {}
	var = engine.execute("select max(date) from {0}.patent;".format(new_database))
	found_date = [row[0] for row in var][0]

	# substitute out - for comparing database timestamps
	found_date = re.sub('-', '', str(found_date))

	if found_date != latest_expected_date:
		date_error =  "The latest date is {0}, and it should be {1}".format(found_date, latest_expected_date)
