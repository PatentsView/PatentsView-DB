...
def transform(folder,host,username,password,appdb,patdb):
    ...
    if patdb:
        ...
        #this line needed to be modified to run on windows
        #inp = csv.reader(file(os.path.join(folder,'application_correct.csv'),'rb'))
        ...
        mydb.commit()
        #drop location_id from raw location (which I accidentally added)
        cursor.execute('update ' + patdb+'.rawlocation set location_id = NULL')
        #clean up excess quotes in text tables
        text_tables = ['brf_sum_text', 'detail_desc_text', 'claim', 'draw_desc_text']
        for table in text_tables:
            cursor.execute('update ' + patdb+'.'+table+' set text=TRIM(BOTH \'"\' FROM text)')
            cursor.execute('update ' + patdb+'.'+table+' set text=TRIM(text)')
        mydb.commit()
        #change all NULL sting to actual mysql NULL
        cursor.execute("SHOW tables FROM " + patdb)
        tables = [table[0] for table in cursor.fetchall()]
        for table in tables:
            cursor.execute("SHOW columns FROM " + patdb +'.' + table)
            columns = [column[0] for column in cursor.fetchall()]
            for column in columns:
                print table, column
                cursor.execute('update ' + patdb+'.'+table+' set `' + column + '` = NULL where `' + column + '` = "NULL"')
        mydb.commit()
