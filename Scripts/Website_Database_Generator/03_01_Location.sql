...
from
  `{{params.reporting_database}}`.`temp_id_mapping_location_transformed` timl
  inner join `{{params.raw_database}}`.`location_assignee` la on la.`location_id` = timl.`old_location_id_transformed`
group by
  timl.`new_location_id`;
...
from
  `{{params.reporting_database}}`.`temp_id_mapping_location` timl
...
# 15:00
alter table `{{params.reporting_database}}`.`temp_location_patent` add index (`location_id`, `patent_id`);
alter table `{{params.reporting_database}}`.`temp_location_patent` add index (`patent_id`, `location_id`);
...
from
  `{{params.reporting_database}}`.`temp_location_patent`
group by
  `location_id`;
...
from
  `{{params.raw_database}}`.`location` l
  inner join `{{params.reporting_database}}`.`temp_id_mapping_location` timl on timl.`old_location_id` = l.`id`
  left outer join `{{params.reporting_database}}`.`temp_id_mapping_location_transformed` timlt on timlt.`new_location_id` = timl.`new_location_id`
  left outer join `{{params.reporting_database}}`.`temp_location_num_assignees` tlna on tlna.`location_id` = timl.`new_location_id`
  left outer join `{{params.reporting_database}}`.`temp_location_num_inventors` tlni on tlni.`location_id` = timl.`new_location_id`
  left outer join `{{params.reporting_database}}`.`temp_location_num_patents` tlnp on tlnp.`location_id` = timl.`new_location_id`;
