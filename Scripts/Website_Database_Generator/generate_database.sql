...
#  source database:       `patent_dev`
#  destination database:  `PatentsView_dev`
...
drop database if exists `PatentsView_dev`;
...
from
  `patent_dev`.`patent` p
  left outer join `patent_dev`.`rawassignee` ra on ra.`patent_id` = p.`id` and ra.`sequence` = 0
...
from
  `patent_dev`.`patent` p
  left outer join `patent_dev`.`rawinventor` ri on ri.`patent_id` = p.`id` and ri.`sequence` = 0
...
select
  `patent_id`, count(*)
from
  `patent_dev`.`usapplicationcitation`
group by
  `patent_id`;
...
create table `PatentsView_dev`.`application`
(
...
# 5,425,879 @ 1:11
insert into `PatentsView_dev`.`application`
...
create table `PatentsView_dev`.`temp_location_num_assignees`
(
...
# 34,018 @ 0:02
insert into `PatentsView_dev`.`temp_location_num_assignees`
  (`location_id`, `num_assignees`)
select
  timl.`new_location_id`,
  count(distinct la.`assignee_id`)
from
  `PatentsView_dev`.`temp_id_mapping_location_transformed` timl
  inner join `patent_dev`.`location_assignee` la on la.`location_id` = timl.`old_location_id_transformed`
group by
  timl.`new_location_id`;


drop table if exists `PatentsView_dev`.`temp_location_num_inventors`;
create table `PatentsView_dev`.`temp_location_num_inventors`
(
  `location_id` int unsigned not null,
...
# 94,350 @ 0:50
insert into `PatentsView_dev`.`temp_location_num_inventors`
...
from
  `PatentsView_dev`.`temp_id_mapping_location_transformed` timl
  inner join `patent_dev`.`location_inventor` li on li.`location_id` = timl.`old_location_id_transformed`
group by
  timl.`new_location_id`;
...
drop table if exists `PatentsView_dev`.`temp_location_patent`;
create table `PatentsView_dev`.`temp_location_patent`
(
  `location_id` int unsigned not null,
  `patent_id` varchar(20) not null
)
...
# 11,867,513 @ 3:41
insert into `PatentsView_dev`.`temp_location_patent`
  (`location_id`, `patent_id`)
select
  timl.`new_location_id`,
  ri.`patent_id`
from
  `PatentsView_dev`.`temp_id_mapping_location` timl
  inner join `patent_dev`.`rawlocation` rl on rl.`location_id` = timl.`old_location_id`
  inner join `patent_dev`.`rawinventor` ri on ri.`rawlocation_id` = rl.`id`;
...
from
  `PatentsView_dev`.`temp_id_mapping_location` timl
  inner join `patent_dev`.`rawlocation` rl on rl.`location_id` = timl.`old_location_id`
  inner join `patent_dev`.`rawassignee` ra on ra.`rawlocation_id` = rl.`id`;


# 15:00
alter table `PatentsView_dev`.`temp_location_patent` add index (`location_id`, `patent_id`);
alter table `PatentsView_dev`.`temp_location_patent` add index (`patent_id`, `location_id`);


drop table if exists `PatentsView_dev`.`temp_location_num_patents`;
create table `PatentsView_dev`.`temp_location_num_patents`
(
  `location_id` int unsigned not null,
...
# 121,475 @ 1:10
insert into `PatentsView_dev`.`temp_location_num_patents`
  (`location_id`, `num_patents`)
select
  `location_id`,
  count(distinct patent_id)
from
  `PatentsView_dev`.`temp_location_patent`
group by
  `location_id`;


drop table if exists `PatentsView_dev`.`location`;
create table `PatentsView_dev`.`location`
(
  `location_id` int unsigned not null,
  `city` varchar(128) null,
  `state` varchar(20) null,
...
# 121,477 @ 0:02
insert into `PatentsView_dev`.`location`
(
...
from
  `patent_dev`.`location` l
  inner join `PatentsView_dev`.`temp_id_mapping_location` timl on timl.`old_location_id` = l.`id`
  left outer join `PatentsView_dev`.`temp_id_mapping_location_transformed` timlt on timlt.`new_location_id` = timl.`new_location_id`
  left outer join `PatentsView_dev`.`temp_location_num_assignees` tlna on tlna.`location_id` = timl.`new_location_id`
  left outer join `PatentsView_dev`.`temp_location_num_inventors` tlni on tlni.`location_id` = timl.`new_location_id`
    ...
    from
	  ...
	  t.`location_id`,
        ...
        from
	      ...
	      rl.`location_id_transformed`
            from
              `patent_dev`.`rawassignee` ra
              inner join `patent_dev`.`patent` p on p.`id` = ra.`patent_id`
              inner join `patent_dev`.`rawlocation` rl on rl.`id` = ra.`rawlocation_id`
            where
...
from
  `patent_dev`.`patent_assignee` aa
  join `patent_dev`.`patent_inventor` ii on ii.patent_id = aa.patent_id
group by
  aa.`assignee_id`;
...
# 4,825,748 @ 7:20
insert into `PatentsView_dev`.`patent_assignee`
(
...
drop table if exists `PatentsView_dev`.`location_assignee`;
create table `PatentsView_dev`.`location_assignee`
(
  `location_id` int unsigned not null,
  `assignee_id` int unsigned not null,
...
# 438,452 @ 0:07
insert into `PatentsView_dev`.`location_assignee`
...
select distinct
  ...
  null
from
  `patent_dev`.`location_assignee` la
  inner join `PatentsView_dev`.`temp_id_mapping_location_transformed` timl on timl.`old_location_id_transformed` = la.`location_id`
  inner join `PatentsView_dev`.`temp_id_mapping_assignee` tima on tima.`old_assignee_id` = la.`assignee_id`;
    ...
    from
	  ...
	  t.`location_id_transformed`
        from
	      ...
	      rl.`location_id_transformed`
            from
              `patent_dev`.`rawinventor` ri
              inner join `patent_dev`.`patent` p on p.`id` = ri.`patent_id`
              inner join `patent_dev`.`rawlocation` rl on rl.`id` = ri.`rawlocation_id`
            where
...
from
  `patent_dev`.`patent_inventor` ii
  join `patent_dev`.`patent_assignee` aa
...
drop table if exists `PatentsView_dev`.`patent_inventor`;
create table `PatentsView_dev`.`patent_inventor`
(
  `patent_id` varchar(20) not null,
...
# 12,389,559 @ 29:50
insert into `PatentsView_dev`.`patent_inventor`
(
...
drop table if exists `PatentsView_dev`.`location_inventor`;
create table `PatentsView_dev`.`location_inventor`
(
  `location_id` int unsigned not null,
  `inventor_id` int unsigned not null,
  `num_patents` int unsigned,
...
# 4,188,507 @ 0:50
insert into `PatentsView_dev`.`location_inventor`
...
select distinct
  ...
  null
from
  `patent_dev`.`location_inventor` la
  inner join `PatentsView_dev`.`temp_id_mapping_location_transformed` timl on timl.`old_location_id_transformed` = la.`location_id`
  inner join `PatentsView_dev`.`temp_id_mapping_inventor` timi on timi.`old_inventor_id` = la.`inventor_id`;
...
create table `PatentsView_dev`.`usapplicationcitation`
(
...
# 13,617,656 @ 8:22
insert into `PatentsView_dev`.`usapplicationcitation`
(
...
create table `PatentsView_dev`.`uspatentcitation`
(
  `citing_patent_id` varchar(20) not null,
  `sequence` int not null,
...
# 71,126,097 @ 32:52
insert into `PatentsView_dev`.`uspatentcitation`
...
create table `PatentsView_dev`.`temp_cpc_current_subsection_aggregate_counts`
(
...
# 29:37
insert into `PatentsView_dev`.`temp_cpc_current_subsection_aggregate_counts`
(
...
select
  ...
  ifnull(round(timestampdiff(day, min(p.`date`), max(p.`date`)) / 365), 0)
from
  `patent_dev`.`cpc_current` c
  left outer join `patent_dev`.`patent_assignee` pa on pa.`patent_id` = c.`patent_id`
  left outer join `patent_dev`.`patent_inventor` pii on pii.`patent_id` = c.`patent_id`
  left outer join `PatentsView_dev`.`patent` p on p.`patent_id` = c.`patent_id`
group by
  c.`subsection_id`;

drop table if exists `PatentsView_dev`.`temp_cpc_subsection_title`;
create table `PatentsView_dev`.`temp_cpc_subsection_title`
(
...
# 0.125 sec
insert into `PatentsView_dev`.`temp_cpc_subsection_title`
  (`id`, `title`)
select
  `id`,
...
from
  `patent_dev`.`cpc_subsection`;


drop table if exists `PatentsView_dev`.`temp_cpc_current_group_aggregate_counts`;
create table `PatentsView_dev`.`temp_cpc_current_group_aggregate_counts`
(
...
# 29:37
insert into `PatentsView_dev`.`temp_cpc_current_group_aggregate_counts`
(
...
drop table if exists `PatentsView_dev`.`temp_cpc_group_title`;
create table `PatentsView_dev`.`temp_cpc_group_title`
(
  `id` varchar(20) not null,
  `title` varchar(256) null,
  primary key (`id`)
...
# 0.156
insert into `PatentsView_dev`.`temp_cpc_group_title`
  (`id`, `title`)
select
  `id`,
  case when binary replace(`title`, 'e.g.', 'E.G.') = binary ucase(`title`)
...
from
  `patent_dev`.`cpc_group`;


drop table if exists `PatentsView_dev`.`temp_cpc_subgroup_title`;
create table `PatentsView_dev`.`temp_cpc_subgroup_title`
(
  `id` varchar(20) not null,
...
# 0:07
insert into `PatentsView_dev`.`temp_cpc_subgroup_title`
  (`id`, `title`)
select
  `id`,
  case when binary replace(`title`, 'e.g.', 'E.G.') = binary ucase(`title`)
...
from
  `patent_dev`.`cpc_subgroup`;


drop table if exists `PatentsView_dev`.`cpc_current`;
create table `PatentsView_dev`.`cpc_current`
(
  `patent_id` varchar(20) not null,
...
# 23,151,381 @ 1:29:48
# 23,151,381 @ 36:32
insert into `PatentsView_dev`.`cpc_current`
(
...
from
  `PatentsView_dev`.`patent` p
  inner join `patent_dev`.`cpc_current` c on p.`patent_id` = c.`patent_id`
  left outer join `PatentsView_dev`.`temp_cpc_subsection_title` s on s.`id` = c.`subsection_id`
  left outer join `PatentsView_dev`.`temp_cpc_group_title` g on g.`id` = c.`group_id`
  left outer join `PatentsView_dev`.`temp_cpc_subgroup_title` sg on sg.`id` = c.`subgroup_id`
  ...
  left outer join `PatentsView_dev`.`temp_cpc_current_group_aggregate_counts` tccgac on tccgac.`group_id` = c.`group_id`;


drop table if exists `PatentsView_dev`.`cpc_current_subsection`;
create table `PatentsView_dev`.`cpc_current_subsection`
(
  `patent_id` varchar(20) not null,
  `section_id` varchar(10) null,
  `subsection_id` varchar(20) null,
...
# 7,240,381 @ 19:00
insert into `PatentsView_dev`.`cpc_current_subsection`
(
...
select
  ...
  case when tccsac.`actual_years_active` < 1 then 1 else tccsac.`actual_years_active` end
from
  (select distinct `patent_id`, `section_id`, `subsection_id` from `PatentsView_dev`.`cpc_current`) c
  left outer join `PatentsView_dev`.`temp_cpc_subsection_title` s on s.`id` = c.`subsection_id`
  left outer join `PatentsView_dev`.`temp_cpc_current_subsection_aggregate_counts` tccsac on tccsac.`subsection_id` = c.`subsection_id`;
...
drop table if exists `PatentsView_dev`.`cpc_current_group`;
create table `PatentsView_dev`.`cpc_current_group`
(
  `patent_id` varchar(20) not null,
  `section_id` varchar(10) null,
  `group_id` varchar(20) null,
...
# 7,240,381 @ 19:00
insert into `PatentsView_dev`.`cpc_current_group`
(
...
select
  ...
  case when tccgac.`actual_years_active` < 1 then 1 else tccgac.`actual_years_active` end
from
  (select distinct `patent_id`, `section_id`, `group_id` from `PatentsView_dev`.`cpc_current`) c
  left outer join `PatentsView_dev`.`temp_cpc_group_title` s on s.`id` = c.`group_id`
  left outer join `PatentsView_dev`.`temp_cpc_current_group_aggregate_counts` tccgac on tccgac.`group_id` = c.`group_id`;
...
create table `PatentsView_dev`.`cpc_current_subsection_patent_year`
(
  `subsection_id` varchar(20) not null,
...
# 13:24
insert into `PatentsView_dev`.`cpc_current_subsection_patent_year`
  (`subsection_id`, `patent_year`, `num_patents`)
select
  c.`subsection_id`, year(p.`date`), count(distinct c.`patent_id`)
from
  `patent_dev`.`cpc_current` c
  inner join `PatentsView_dev`.`patent` p on p.`patent_id` = c.`patent_id` and p.`date` is not null
where
...
drop table if exists `PatentsView_dev`.`cpc_current_group_patent_year`;
create table `PatentsView_dev`.`cpc_current_group_patent_year`
(
  `group_id` varchar(20) not null,
  `patent_year` smallint unsigned not null,
  `num_patents` int unsigned not null,
...
# 13:24
insert into `PatentsView_dev`.`cpc_current_group_patent_year`
...
create table `PatentsView_dev`.`temp_ipcr_aggregations`
(
...
# 11:53
insert into `PatentsView_dev`.`temp_ipcr_aggregations`
...
from
  `patent_dev`.`ipcr` i
  left outer join `patent_dev`.`patent_assignee` pa on pa.`patent_id` = i.`patent_id`
  left outer join `patent_dev`.`patent_inventor` pii on pii.`patent_id` = i.`patent_id`
group by
  i.`section`, i.`ipc_class`, i.`subclass`;


drop table if exists `PatentsView_dev`.`temp_ipcr_years_active`;
create table `PatentsView_dev`.`temp_ipcr_years_active`
(
  `section` varchar(20) null,
  `ipc_class` varchar(20) null,
  `subclass` varchar(20) null,
...
# 2:17
insert into `PatentsView_dev`.`temp_ipcr_years_active`
(
...
from
  `patent_dev`.`ipcr` i
  inner join `PatentsView_dev`.`patent` p on p.`patent_id`= i.`patent_id`
where
  p.`date` is not null
group by
  i.`section`, i.`ipc_class`, i.`subclass`;
...
drop table if exists `PatentsView_dev`.`ipcr`;
create table `PatentsView_dev`.`ipcr`
(
  `patent_id` varchar(20) not null,
  `sequence` int not null,
  `section` varchar(20) null,
...
# 7,702,885 @ 6:38
insert into `PatentsView_dev`.`ipcr`
(
...
create table `PatentsView_dev`.`temp_nber_subcategory_aggregate_counts`
(
...
# 38 @ 4:45
insert into `PatentsView_dev`.`temp_nber_subcategory_aggregate_counts`
(
...
from
  `patent_dev`.`nber` n
  left outer join `patent_dev`.`patent_assignee` pa on pa.`patent_id` = n.`patent_id`
  left outer join `patent_dev`.`patent_inventor` pii on pii.`patent_id` = n.`patent_id`
  left outer join `PatentsView_dev`.`patent` p on p.`patent_id` = n.`patent_id`
group by
  n.`subcategory_id`;


drop table if exists `PatentsView_dev`.`nber`;
create table `PatentsView_dev`.`nber`
(
  `patent_id` varchar(20) not null,
...
# 4,927,287 @ 1:47
insert into `PatentsView_dev`.`nber`
(
...
from
  `PatentsView_dev`.`patent` p
  inner join `patent_dev`.`nber` n on p.`patent_id` = n.`patent_id`
  left outer join `patent_dev`.`nber_category` c on c.`id` = n.`category_id`
  left outer join `patent_dev`.`nber_subcategory` s on s.`id` = n.`subcategory_id`
...
create table `PatentsView_dev`.`nber_subcategory_patent_year`
(
  `subcategory_id` varchar(20) not null,
  `patent_year` smallint unsigned not null,
  `num_patents` int unsigned not null,
...
# 1,483 @ 1:01
insert into `PatentsView_dev`.`nber_subcategory_patent_year`
  (`subcategory_id`, `patent_year`, `num_patents`)
select
  n.`subcategory_id`, year(p.`date`), count(distinct n.`patent_id`)
from
  `patent_dev`.`nber` n
  inner join `PatentsView_dev`.`patent` p on p.`patent_id` = n.`patent_id` and p.`date` is not null
where
...
create table `PatentsView_dev`.`temp_mainclass_current_aggregate_counts`
(
...
# 24:52
insert into `PatentsView_dev`.`temp_mainclass_current_aggregate_counts`
(
...
select
  ...
  ifnull(round(timestampdiff(day, min(p.`date`), max(p.`date`)) / 365), 0)
from
  `patent_dev`.`uspc_current` u
  left outer join `patent_dev`.`patent_assignee` pa on pa.`patent_id` = u.`patent_id`
  left outer join `patent_dev`.`patent_inventor` pii on pii.`patent_id` = u.`patent_id`
  left outer join `PatentsView_dev`.`patent` p on p.`patent_id` = u.`patent_id` and p.`date` is not null
where
...
drop table if exists `PatentsView_dev`.`temp_mainclass_current_title`;
create table `PatentsView_dev`.`temp_mainclass_current_title`
(
  `id` varchar(20) not null,
  `title` varchar(512) null,
  primary key (`id`)
...
# 0.125 sec
insert into `PatentsView_dev`.`temp_mainclass_current_title`
  (`id`, `title`)
select
  `id`,
  case when binary replace(`title`, 'e.g.', 'E.G.') = binary ucase(`title`)
...
from
  `patent_dev`.`mainclass_current`;


# Fix casing of subclass_current.
drop table if exists `PatentsView_dev`.`temp_subclass_current_title`;
create table `PatentsView_dev`.`temp_subclass_current_title`
(
  `id` varchar(20) not null,
  `title` varchar(512) null,
  primary key (`id`)
...
# "Fix" casing where necessary.
# 1.719 sec
insert into `PatentsView_dev`.`temp_subclass_current_title`
  (`id`, `title`)
select
  `id`,
  case when binary replace(`title`, 'e.g.', 'E.G.') = binary ucase(`title`)
...
from
  `patent_dev`.`subclass_current`;


drop table if exists `PatentsView_dev`.`uspc_current`;
create table `PatentsView_dev`.`uspc_current`
(
  `patent_id` varchar(20) not null,
  `sequence` int unsigned not null,
...
# 21,175,812 @ 1:02:06
# 21,175,812 @ 11:36
insert into `PatentsView_dev`.`uspc_current`
(
...
from
  `PatentsView_dev`.`patent` p
  inner join `patent_dev`.`uspc_current` u on u.`patent_id` = p.`patent_id`
  left outer join `PatentsView_dev`.`temp_mainclass_current_title` m on m.`id` = u.`mainclass_id`
  left outer join `PatentsView_dev`.`temp_subclass_current_title` s on s.`id` = u.`subclass_id`
  left outer join `PatentsView_dev`.`temp_mainclass_current_aggregate_counts` tmcac on tmcac.`mainclass_id` = u.`mainclass_id`;


drop table if exists `PatentsView_dev`.`uspc_current_mainclass`;
create table `PatentsView_dev`.`uspc_current_mainclass`
(
  `patent_id` varchar(20) not null,
  `mainclass_id` varchar(20) null,
  `mainclass_title` varchar(256) null,
...
# 9,054,003 @ 9:27
insert into `PatentsView_dev`.`uspc_current_mainclass`
(
...
select
  ...
  ifnull(case when tmcac.`actual_years_active` < 1 then 1 else tmcac.`actual_years_active` end, 0)
from
  (select distinct `patent_id`, `mainclass_id` from `PatentsView_dev`.`uspc_current`) u
  left outer join `PatentsView_dev`.`temp_mainclass_current_title` m on m.`id` = u.`mainclass_id`
  left outer join `PatentsView_dev`.`temp_mainclass_current_aggregate_counts` tmcac on tmcac.`mainclass_id` = u.`mainclass_id`;
...
create table `PatentsView_dev`.`uspc_current_mainclass_application_year`
(
  `mainclass_id` varchar(20) not null,
...
# 20,241 @ 0:56
insert into `PatentsView_dev`.`uspc_current_mainclass_application_year`
...
from
  `PatentsView_dev`.`patent` p
  inner join `PatentsView_dev`.`uspc_current` u on u.`patent_id` = p.`patent_id`
where
...
update
  `PatentsView_dev`.`patent` p
  inner join `PatentsView_dev`.`uspc_current` u on
    u.`patent_id` = p.`patent_id` and u.`sequence` = 0
  inner join `PatentsView_dev`.`uspc_current_mainclass_application_year` c on
...
drop table if exists `PatentsView_dev`.`cpc_current_group_application_year`;
create table `PatentsView_dev`.`cpc_current_group_application_year`
(
  `group_id` varchar(20) not null,
  `application_year` smallint unsigned not null,
  `sample_size` int unsigned not null,
...
# 20,241 @ 0:56
insert into `PatentsView_dev`.`cpc_current_group_application_year`
...
from
  `PatentsView_dev`.`patent` p
  inner join `PatentsView_dev`.`cpc_current` u on u.`patent_id` = p.`patent_id`
where
  p.`patent_processing_days` is not null and u.`sequence` = 0
group by
...
update
  `PatentsView_dev`.`patent` p
  inner join `PatentsView_dev`.`cpc_current` u on
    u.`patent_id` = p.`patent_id` and u.`sequence` = 0
  inner join `PatentsView_dev`.`cpc_current_group_application_year` c on
...
create table `PatentsView_dev`.`uspc_current_mainclass_patent_year`
(
  `mainclass_id` varchar(20) not null,
  `patent_year` smallint unsigned not null,
  `num_patents` int unsigned not null,
...
# 18,316 @ 12:56
insert into `PatentsView_dev`.`uspc_current_mainclass_patent_year`
  (`mainclass_id`, `patent_year`, `num_patents`)
select
  u.`mainclass_id`, year(p.`date`), count(distinct u.`patent_id`)
from
  `patent_dev`.`uspc_current` u
  inner join `PatentsView_dev`.`patent` p on p.`patent_id` = u.`patent_id` and p.`date` is not null
where
  u.`mainclass_id` is not null and u.`mainclass_id` != ''
group by
...
drop table if exists `PatentsView_dev`.`assignee_inventor`;
create table `PatentsView_dev`.`assignee_inventor`
(
  `assignee_id` int unsigned not null,
  `inventor_id` int unsigned not null,
  `num_patents` int unsigned not null
...
# 4,352,502 @ 1:52
insert into `PatentsView_dev`.`assignee_inventor`
  (`assignee_id`, `inventor_id`, `num_patents`)
select
  pa.assignee_id, pi.inventor_id, count(distinct pa.patent_id)
from
  `PatentsView_dev`.`patent_assignee` pa
  inner join `PatentsView_dev`.`patent_inventor` pi using(patent_id)
group by
...
create table `PatentsView_dev`.`inventor_coinventor`
(
  `inventor_id` int unsigned not null,
...
# 16,742,248 @ 11:55
insert into `PatentsView_dev`.`inventor_coinventor`
  (`inventor_id`, `coinventor_id`, `num_patents`)
select
  pi.inventor_id, copi.inventor_id, count(distinct copi.patent_id)
from
  `PatentsView_dev`.`patent_inventor` pi
  inner join `PatentsView_dev`.`patent_inventor` copi on pi.patent_id=copi.patent_id and pi.inventor_id<>copi.inventor_id
group by
...
create table `PatentsView_dev`.`inventor_cpc_subsection`
(
  `inventor_id` int unsigned not null,
  `subsection_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 7,171,415 @ 11:55
insert into `PatentsView_dev`.`inventor_cpc_subsection`
  (`inventor_id`, `subsection_id`, `num_patents`)
select
  pi.inventor_id, c.subsection_id, count(distinct c.patent_id)
from
  `PatentsView_dev`.`patent_inventor` pi
  inner join `PatentsView_dev`.`cpc_current_subsection` c using(patent_id)
where
  c.subsection_id is not null and c.subsection_id != ''
group by
...
drop table if exists `PatentsView_dev`.`inventor_cpc_group`;
create table `PatentsView_dev`.`inventor_cpc_group`
(
  `inventor_id` int unsigned not null,
  `group_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 7,171,415 @ 11:55
insert into `PatentsView_dev`.`inventor_cpc_group`
  (`inventor_id`, `group_id`, `num_patents`)
select
  pi.inventor_id, c.group_id, count(distinct c.patent_id)
from
  `PatentsView_dev`.`patent_inventor` pi
  inner join `PatentsView_dev`.`cpc_current_group` c using(patent_id)
where
  c.group_id is not null and c.group_id != ''
group by
...
create table `PatentsView_dev`.`inventor_nber_subcategory`
(
  `inventor_id` int unsigned not null,
  `subcategory_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
create table `PatentsView_dev`.`inventor_uspc_mainclass`
(
  `inventor_id` int unsigned not null,
  `mainclass_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 10,350,577 @ 14:44
insert into `PatentsView_dev`.`inventor_uspc_mainclass`
  (`inventor_id`, `mainclass_id`, `num_patents`)
select
  pi.inventor_id, u.mainclass_id, count(distinct pi.patent_id)
from
  `PatentsView_dev`.`patent_inventor` pi
  inner join `PatentsView_dev`.`uspc_current_mainclass` u on pi.patent_id=u.patent_id
group by
...
drop table if exists `PatentsView_dev`.`inventor_year`;
create table `PatentsView_dev`.`inventor_year`
(
  `inventor_id` int unsigned not null,
...
# 8,140,017 @ 2:19
insert into `PatentsView_dev`.`inventor_year`
(`inventor_id`, `patent_year`, `num_patents`)
select
  pi.inventor_id, p.year, count(distinct pi.patent_id)
from
  `PatentsView_dev`.`patent_inventor` pi
  inner join `PatentsView_dev`.`patent` p using(patent_id)
group by
...
create table `PatentsView_dev`.`assignee_cpc_subsection`
(
  `assignee_id` int unsigned not null,
  `subsection_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 933,903 @ 2:22
insert into `PatentsView_dev`.`assignee_cpc_subsection`
...
drop table if exists `PatentsView_dev`.`assignee_cpc_group`;
create table `PatentsView_dev`.`assignee_cpc_group`
(
  `assignee_id` int unsigned not null,
  `group_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 933,903 @ 2:22
insert into `PatentsView_dev`.`assignee_cpc_group`
...
create table `PatentsView_dev`.`assignee_nber_subcategory`
(
  `assignee_id` int unsigned not null,
  `subcategory_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 618,873 @ 0:48
insert into `PatentsView_dev`.`assignee_nber_subcategory`
  (`assignee_id`, `subcategory_id`, `num_patents`)
select
  pa.assignee_id, n.subcategory_id, count(distinct n.patent_id)
from
  `PatentsView_dev`.`patent_assignee` pa
  inner join `PatentsView_dev`.`nber` n using(patent_id)
where
  n.subcategory_id is not null and n.subcategory_id != ''
group by
...
create table `PatentsView_dev`.`assignee_uspc_mainclass`
(
  `assignee_id` int unsigned not null,
  `mainclass_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 1,534,644 @ 3:30
insert into `PatentsView_dev`.`assignee_uspc_mainclass`
  (`assignee_id`, `mainclass_id`, `num_patents`)
select
  pa.assignee_id, u.mainclass_id, count(distinct pa.patent_id)
from
  `PatentsView_dev`.`patent_assignee` pa
  inner join `PatentsView_dev`.`uspc_current_mainclass` u on pa.patent_id=u.patent_id
group by
...
drop table if exists `PatentsView_dev`.`assignee_year`;
create table `PatentsView_dev`.`assignee_year`
(
  `assignee_id` int unsigned not null,
  `patent_year` smallint not null,
  `num_patents` int unsigned not null
...
# 931,856 @ 2:00
insert into `PatentsView_dev`.`assignee_year`
...
# 434,823 @ 0:17
update
  `PatentsView_dev`.`location_assignee` la
  inner join
  (
    select
      `location_id`, `assignee_id`, count(distinct `patent_id`) num_patents
    from
      `PatentsView_dev`.`patent_assignee`
    group by
...
# 4,167,939 @ 2:33
update
  `PatentsView_dev`.`location_inventor` li
  inner join
  (
    select
      `location_id`, `inventor_id`, count(distinct `patent_id`) num_patents
    from
      `PatentsView_dev`.`patent_inventor`
    group by
...
create table `PatentsView_dev`.`location_cpc_subsection`
(
  `location_id` int unsigned not null,
  `subsection_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 1,077,971 @ 6:19
insert into `PatentsView_dev`.`location_cpc_subsection`
  (`location_id`, `subsection_id`, `num_patents`)
select
  tlp.`location_id`, cpc.`subsection_id`, count(distinct tlp.`patent_id`)
from
  `PatentsView_dev`.`temp_location_patent` tlp
  inner join `PatentsView_dev`.`cpc_current_subsection` cpc using(`patent_id`)
group by
...
drop table if exists `PatentsView_dev`.`location_cpc_group`;
create table `PatentsView_dev`.`location_cpc_group`
(
  `location_id` int unsigned not null,
  `group_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 1,077,971 @ 6:19
insert into `PatentsView_dev`.`location_cpc_group`
  (`location_id`, `group_id`, `num_patents`)
select
  tlp.`location_id`, cpc.`group_id`, count(distinct tlp.`patent_id`)
from
  `PatentsView_dev`.`temp_location_patent` tlp
  inner join `PatentsView_dev`.`cpc_current_group` cpc using(`patent_id`)
group by
...
create table `PatentsView_dev`.`location_uspc_mainclass`
(
  `location_id` int unsigned not null,
  `mainclass_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 2,260,351 @ 7:47
insert into `PatentsView_dev`.`location_uspc_mainclass`
  (`location_id`, `mainclass_id`, `num_patents`)
select
  tlp.`location_id`, uspc.`mainclass_id`, count(distinct tlp.`patent_id`)
from
  `PatentsView_dev`.`temp_location_patent` tlp
  inner join `PatentsView_dev`.`uspc_current_mainclass` uspc using(`patent_id`)
group by
...
create table `PatentsView_dev`.`location_nber_subcategory`
(
  `location_id` int unsigned not null,
  `subcategory_id` varchar(20) not null,
  `num_patents` int unsigned not null
...
# 
insert into `PatentsView_dev`.`location_nber_subcategory`
  (`location_id`, `subcategory_id`, `num_patents`)
select
  tlp.`location_id`, nber.`subcategory_id`, count(distinct tlp.`patent_id`)
from
  `PatentsView_dev`.`temp_location_patent` tlp
  inner join `PatentsView_dev`.`nber` nber using(`patent_id`)
group by
...
drop table if exists `PatentsView_dev`.`location_year`;
create table `PatentsView_dev`.`location_year`
(
  `location_id` int unsigned not null,
...
# 867,942 @ 1:19
insert into `PatentsView_dev`.`location_year`
...
# 1:53:23
alter table `PatentsView_dev`.`application` add index `ix_application_number` (`number`);
alter table `PatentsView_dev`.`application` add index `ix_application_patent_id` (`patent_id`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_name_first` (`name_first`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_name_last` (`name_last`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_organization` (`organization`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_persistent_assignee_id` (`persistent_assignee_id`);
alter table `PatentsView_dev`.`assignee_cpc_subsection` add index `ix_assignee_cpc_subsection_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`assignee_cpc_subsection` add index `ix_assignee_cpc_subsection_subsection_id` (`subsection_id`);
alter table `PatentsView_dev`.`assignee_cpc_group` add index `ix_assignee_cpc_group_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`assignee_cpc_group` add index `ix_assignee_cpc_group_group_id` (`group_id`);
alter table `PatentsView_dev`.`assignee_inventor` add index `ix_assignee_inventor_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`assignee_inventor` add index `ix_assignee_inventor_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`assignee_nber_subcategory` add index `ix_assignee_nber_subcategory_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`assignee_nber_subcategory` add index `ix_assignee_nber_subcategory_subcategory_id` (`subcategory_id`);
alter table `PatentsView_dev`.`assignee_uspc_mainclass` add index `ix_assignee_uspc_mainclass_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`assignee_uspc_mainclass` add index `ix_assignee_uspc_mainclass_mainclass_id` (`mainclass_id`);
alter table `PatentsView_dev`.`assignee_year` add index `ix_assignee_year_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`assignee_year` add index `ix_assignee_year_year` (`patent_year`);
alter table `PatentsView_dev`.`cpc_current_group` add index `ix_cpc_current_group_group_id` (`group_id`);
alter table `PatentsView_dev`.`cpc_current_group` add index `ix_cpc_current_group_title` (`group_title`);
alter table `PatentsView_dev`.`cpc_current_subsection` add index `ix_cpc_current_subsection_subsection_id` (`subsection_id`);
alter table `PatentsView_dev`.`cpc_current_subsection` add index `ix_cpc_current_subsection_title` (`subsection_title`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_group_id` (`group_id`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_subgroup_id` (`subgroup_id`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_subsection_id` (`subsection_id`);
alter table `PatentsView_dev`.`inventor` add index `ix_inventor_name_first` (`name_first`);
alter table `PatentsView_dev`.`inventor` add index `ix_inventor_name_last` (`name_last`);
alter table `PatentsView_dev`.`inventor` add index `ix_inventor_persistent_inventor_id` (`persistent_inventor_id`);
alter table `PatentsView_dev`.`inventor_coinventor` add index `ix_inventor_coinventor_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`inventor_coinventor` add index `ix_inventor_coinventor_coinventor_id` (`coinventor_id`);
alter table `PatentsView_dev`.`inventor_cpc_subsection` add index `ix_inventor_cpc_subsection_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`inventor_cpc_subsection` add index `ix_inventor_cpc_subsection_subsection_id` (`subsection_id`);
alter table `PatentsView_dev`.`inventor_cpc_group` add index `ix_inventor_cpc_group_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`inventor_cpc_group` add index `ix_inventor_cpc_group_group_id` (`group_id`);
alter table `PatentsView_dev`.`inventor_nber_subcategory` add index `ix_inventor_nber_subcategory_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`inventor_nber_subcategory` add index `ix_inventor_nber_subcategory_subcategory_id` (`subcategory_id`);
alter table `PatentsView_dev`.`inventor_uspc_mainclass` add index `ix_inventor_uspc_mainclass_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`inventor_uspc_mainclass` add index `ix_inventor_uspc_mainclass_mainclass_id` (`mainclass_id`);
alter table `PatentsView_dev`.`inventor_year` add index `ix_inventor_year_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`inventor_year` add index `ix_inventor_year_year` (`patent_year`);
alter table `PatentsView_dev`.`ipcr` add index `ix_ipcr_ipc_class` (`ipc_class`);
alter table `PatentsView_dev`.`location_assignee` add index `ix_location_assignee_assignee_id` (`assignee_id`);
alter table `PatentsView_dev`.`location_inventor` add index `ix_location_inventor_inventor_id` (`inventor_id`);
alter table `PatentsView_dev`.`location` add index `ix_location_city` (`city`);
alter table `PatentsView_dev`.`location` add index `ix_location_country` (`country`);
alter table `PatentsView_dev`.`location` add index `ix_location_persistent_location_id` (`persistent_location_id`);
alter table `PatentsView_dev`.`location` add index `ix_location_state` (`state`);
alter table `PatentsView_dev`.`location_cpc_subsection` add index `ix_location_cpc_subsection_location_id` (`location_id`);
alter table `PatentsView_dev`.`location_cpc_subsection` add index `ix_location_cpc_subsection_subsection_id` (`subsection_id`);
alter table `PatentsView_dev`.`location_cpc_group` add index `ix_location_cpc_group_location_id` (`location_id`);
alter table `PatentsView_dev`.`location_cpc_group` add index `ix_location_cpc_group_subsection_id` (`group_id`);
alter table `PatentsView_dev`.`location_nber_subcategory` add index `ix_location_nber_subcategory_location_id` (`location_id`);
alter table `PatentsView_dev`.`location_nber_subcategory` add index `ix_location_nber_subcategory_mainclass_id` (`subcategory_id`);
alter table `PatentsView_dev`.`location_uspc_mainclass` add index `ix_location_uspc_mainclass_location_id` (`location_id`);
alter table `PatentsView_dev`.`location_uspc_mainclass` add index `ix_location_uspc_mainclass_mainclass_id` (`mainclass_id`);
alter table `PatentsView_dev`.`location_year` add index `ix_location_year_location_id` (`location_id`);
alter table `PatentsView_dev`.`location_year` add index `ix_location_year_year` (`year`);
alter table `PatentsView_dev`.`nber` add index `ix_nber_subcategory_id` (`subcategory_id`);
alter table `PatentsView_dev`.`nber` add index `ix_nber_subcategory_title` (`subcategory_title`);
alter table `PatentsView_dev`.`patent_assignee` add index `ix_patent_assignee_location_id` (`location_id`);
alter table `PatentsView_dev`.`patent_inventor` add index `ix_patent_inventor_location_id` (`location_id`);
alter table `PatentsView_dev`.`patent` add index `ix_patent_date` (`date`);
alter table `PatentsView_dev`.`patent` add index `ix_patent_number` (`number`);
alter table `PatentsView_dev`.`patent` add index `ix_patent_title` (`title`(128));
alter table `PatentsView_dev`.`patent` add index `ix_patent_type` (`type`);
alter table `PatentsView_dev`.`patent` add index `ix_patent_year` (`year`);
alter table `PatentsView_dev`.`usapplicationcitation` add index `ix_usapplicationcitation_cited_application_id` (`cited_application_id`);
alter table `PatentsView_dev`.`uspatentcitation` add index `ix_uspatentcitation_cited_patent_id` (`cited_patent_id`);
alter table `PatentsView_dev`.`uspc_current_mainclass` add index `ix_uspc_current_mainclass_mainclass_id` (`mainclass_id`);
alter table `PatentsView_dev`.`uspc_current_mainclass` add index `ix_uspc_current_mainclass_mainclass_title` (`mainclass_title`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_mainclass_id` (`mainclass_id`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_subclass_id` (`subclass_id`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_mainclass_title` (`mainclass_title`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_subclass_title` (`subclass_title`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`assignee_cpc_subsection` add index `ix_assignee_cpc_subsection_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`assignee_cpc_group` add index `ix_assignee_cpc_group_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`assignee_inventor` add index `ix_assignee_inventor_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`assignee_nber_subcategory` add index `ix_assignee_nber_subcategory_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`assignee_uspc_mainclass` add index `ix_assignee_uspc_mainclass_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`assignee_year` add index `ix_assignee_year_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`cpc_current_subsection` add index `ix_cpc_current_subsection_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_current_subsection` add index `ix_cpc_current_subsection_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`cpc_current_subsection` add index `ix_cpc_current_subsection_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`cpc_current_subsection_patent_year` add index `ix_cpc_current_subsection_patent_year_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_num_patents_group` (`num_patents_group`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_num_inventors_group` (`num_inventors_group`);
alter table `PatentsView_dev`.`cpc_current` add index `ix_cpc_current_num_assignees_group` (`num_assignees_group`);
alter table `PatentsView_dev`.`cpc_current_group` add index `ix_cpc_current_group_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_current_group` add index `ix_cpc_current_group_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`cpc_current_group` add index `ix_cpc_current_group_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`cpc_current_group_patent_year` add index `ix_cpc_current_group_patent_year_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor` add index `ix_inventor_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor` add index `ix_inventor_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`inventor_coinventor` add index `ix_inventor_coinventor_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor_cpc_subsection` add index `ix_inventor_cpc_subsection_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor_cpc_group` add index `ix_inventor_cpc_group_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor_nber_subcategory` add index `ix_inventor_nber_subcategory_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor_uspc_mainclass` add index `ix_inventor_uspc_mainclass_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`inventor_year` add index `ix_inventor_year_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`ipcr` add index `ix_ipcr_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`ipcr` add index `ix_ipcr_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`location` add index `ix_location_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location` add index `ix_location_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`location` add index `ix_location_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`location_assignee` add index `ix_location_assignee_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location_cpc_subsection` add index `ix_location_cpc_subsection_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location_cpc_group` add index `ix_location_cpc_group_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location_inventor` add index `ix_location_inventor_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location_nber_subcategory` add index `ix_location_nber_subcategory_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location_uspc_mainclass` add index `ix_location_uspc_mainclass_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`location_year` add index `ix_location_year_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`nber` add index `ix_nber_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`nber` add index `ix_nber_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`nber` add index `ix_nber_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`nber_subcategory_patent_year` add index `ix_nber_subcategory_patent_year_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`uspc_current` add index `ix_uspc_current_num_assignees` (`num_assignees`);
...
alter table `PatentsView_dev`.`inventor` add index `ix_inventor_last_seen_date` (`last_seen_date`);
alter table `PatentsView_dev`.`nber` add index `ix_nber_category_id` (`category_id`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_lastknown_location_id` (`lastknown_location_id`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_lastknown_persistent_location_id` (`lastknown_persistent_location_id`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_last_seen_date` (`last_seen_date`);
alter table `PatentsView_dev`.`assignee` add index `ix_assignee_first_seen_date` (`first_seen_date`);
alter table `PatentsView_dev`.`patent` add index `ix_patent_country` (`country`);
...
create table if not exists `PatentsView_dev`.`cpc_subsection` (id varchar(20) primary key,title varchar(256), num_patents int(10) unsigned, num_inventors int(10) unsigned, num_assignees int(10) unsigned,first_seen_date date,last_seen_date date,years_active smallint(5) unsigned);
create table if not exists `PatentsView_dev`.`cpc_subgroup` (id varchar(20) primary key,title varchar(512));
create table if not exists `PatentsView_dev`.`cpc_group` (id varchar(20) primary key,title varchar(256), num_patents int(10) unsigned, num_inventors int(10) unsigned, num_assignees int(10) unsigned,first_seen_date date,last_seen_date date,years_active smallint(5) unsigned);
...
CREATE TABLE if not exists `PatentsView_dev`.`nber_copy` ( `patent_id` varchar(20) NOT NULL,  `category_id` varchar(20) DEFAULT NULL, 
...
CREATE TABLE if not exists `PatentsView_dev`.`cpc_current_copy` (  `patent_id` varchar(20) NOT NULL,  `sequence` int(10) unsigned NOT NULL, 
...
CREATE TABLE if not exists `PatentsView_dev`.`cpc_current_subsection_copy` (  `patent_id` varchar(20) NOT NULL,  
...
CREATE TABLE if not exists `PatentsView_dev`.`cpc_current_group_copy` (  `patent_id` varchar(20) NOT NULL,  
...
CREATE TABLE if not exists `PatentsView_dev`.`uspc_current_mainclass_copy` (  `patent_id` varchar(20) NOT NULL,  
...
CREATE TABLE if not exists `PatentsView_dev`.`uspc_current_copy` (  `patent_id` varchar(20) NOT NULL,  `sequence` int(10) unsigned NOT NULL,  
...
insert into `PatentsView_dev`.cpc_group select group_id,group_title,num_patents_group,num_inventors_group,num_assignees_group,first_seen_date_group,last_seen_date_group,years_active_group from `PatentsView_dev`.cpc_current group by group_id;
...
insert into `PatentsView_dev`.cpc_current_group_copy select distinct patent_id,section_id,group_id from `PatentsView_dev`.cpc_current_group;
...
alter table `PatentsView_dev`.`uspc_mainclass` add index `ix_uspc_mainclass_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`uspc_mainclass` add index `ix_uspc_mainclass_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`uspc_mainclass` add index `ix_uspc_mainclass_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`nber_subcategory` add index `ix_nber_subcategory_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`nber_subcategory` add index `ix_nber_subcategory_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`nber_subcategory` add index `ix_nber_subcategory_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_subsection` add index `ix_cpc_subsection_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`cpc_subsection` add index `ix_cpc_subsection_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`cpc_subsection` add index `ix_cpc_subsection_num_patents` (`num_patents`);
alter table `PatentsView_dev`.`cpc_group` add index `ix_cpc_group_num_inventors` (`num_inventors`);
alter table `PatentsView_dev`.`cpc_group` add index `ix_cpc_group_num_assignees` (`num_assignees`);
alter table `PatentsView_dev`.`cpc_group` add index `ix_cpc_group_num_patents` (`num_patents`);
