...
select distinct
  `inventor_id`
from
  `patent_20171226`.`patent_inventor`;
...
select
  `patent_id`, count(*)
from
  `patent_20171226`.`foreigncitation`
group by
  `patent_id`;
...
select
  `patent_id`, count(*)
from
  `patent_20171226`.`usapplicationcitation`
group by
  `patent_id`;
...
select
  `patent_id`, count(*)
from
  `patent_20171226`.`uspatentcitation`
group by
  `patent_id`;
    ...
    from
	  ...
	  t.`location_id`,
        ...
        from
	      ...
	      rl.`location_id_transformed`
            from
              `patent_20171226`.`rawassignee` ra
              inner join `patent_20171226`.`patent` p on p.`id` = ra.`patent_id`
              inner join `patent_20171226`.`rawlocation` rl on rl.`id` = ra.`rawlocation_id`
            where
...
select
  ...
  count(distinct `patent_id`)
from
  `patent_20171226`.`patent_assignee`
group by
  `assignee_id`;
...
from
  `patent_20171226`.`patent_assignee` aa
  join `patent_20171226`.`patent_inventor` ii on ii.patent_id = aa.patent_id
group by
  aa.`assignee_id`;
...
on t.`patent_id` = pa.`patent_id` and t.`assignee_id` = pa.`assignee_id`
  left join `patent_20171226`.`rawassignee` ra on ra.`patent_id` = t.`patent_id` and ra.`assignee_id` = t.`assignee_id` and ra.`sequence` 
    ...
    from
	  ...
	  t.`location_id_transformed`
        from
	      ...
	      rl.`location_id_transformed`
            from
              `patent_20171226`.`rawinventor` ri
              inner join `patent_20171226`.`patent` p on p.`id` = ri.`patent_id`
              inner join `patent_20171226`.`rawlocation` rl on rl.`id` = ri.`rawlocation_id`
            where
...
from
  `patent_20171226`.`patent_inventor` ii
  join `patent_20171226`.`patent_assignee` aa
...
on t.`patent_id` = pii.`patent_id` and t.`inventor_id` = pii.`inventor_id`
  left outer join `patent_20171226`.`rawinventor` ri on ri.`patent_id` = t.`patent_id` and ri.`inventor_id` = t.`inventor_id` and ri.`sequence` 
...
from
  `patent_20171226`.`patent_lawyer` ii
  join `patent_20171226`.`patent_assignee` aa
  on aa.`patent_id` = ii.`patent_id`
group by
  ii.`lawyer_id`;
...
select
  ...
  count(distinct ii.`inventor_id`)
from
  `patent_20171226`.`patent_lawyer` aa
  join `patent_20171226`.`patent_inventor` ii on ii.patent_id = aa.patent_id
group by
  aa.`lawyer_id`;
...
from
  ...
  left outer join `patent_20171226`.`patent_assignee` pa on pa.`patent_id` = i.`patent_id`
  left outer join `patent_20171226`.`patent_inventor` pii on pii.`patent_id` = i.`patent_id`
group by
  i.`section`, i.`ipc_class`, i.`subclass`;
...
from
  `patent_20171226`.`mainclass_current`;
