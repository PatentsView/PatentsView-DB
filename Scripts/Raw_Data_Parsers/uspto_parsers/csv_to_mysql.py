...
def is_after_2015(patent_id):
    ...
    try:
        ...
        if first_char == 'D':
            ...
            pass #no patents starting the T or H are past the cut off date
        else:
            print patent_id
            return False

def upload_csv(host,username,password,dbname,folder, file = None, tablename = None):
    mydb = MySQLdb.connect(host=host,
        user=username,
        passwd=password,
        db=dbname,
    ...
    if file:
        ...
        cursor.execute("load data local infile '"+file_name +"' into table "+tablename+" fields terminated by '\t' lines terminated by '\r\n' ignore 1 lines")
    mydb.commit()
...
def upload_wipo(host,username,password,database,working_directory):
    mydb = MySQLdb.connect(host=host,
        user=username,
        passwd=password,
        db = database)
    ...
    for c in list(cursor.fetchall()):
        c = list(c)
        cursor.execute('insert into wipo values("'+c[0]+'","'+c[1]+'",0)')
        if num%1000 == 0:
            mydb.commit()
        num+=1
    mydb.commit()
