...
def db_and_table_as_array(single_line_query):
    ...
    if after_into is None or after_from is None:
        ...
        config = get_current_config(
            db_type, **{"execution_date": datetime.date(2022, 1, 1)}
        )
    ...
    if "delete_sql" in context["templates_dict"] and drop_existing:
        ...
        for sql_statement in rm_statements:
            ...
            if len(sqlparse.parse(sql_statement)) > 0:
                ...
                if (
                    not drop_existing
                    and parsed_statement.get_type().lower() == "unknown"
                ):
                ...
                if parsed_statement.get_type().lower() == "insert":
                    ...
                    query_plan_check = database_helpers.check_query_plan(
                        db_con, single_line_query
                    )
                    if not query_plan_check:
                        message = """
                            Query execution plan involves full table scan: ```{single_line_query} ```
                            """.format(
                            single_line_query=single_line_query
                        )
        ...
        for sql_statement in sql_statements:
            ...
            if len(sqlparse.parse(sql_statement)) > 0:
                ...
                if parsed_statement.get_type().lower() == "insert":
                    ...
                    collation_check_parameters = db_and_table_as_array(
                        single_line_query
                    )
                    ...
                    if not database_helpers.check_encoding_and_collation(
                        db_con, collation_check_parameters
                    ):
                        message = """
...
if __name__ == "__main__":
    ...
    template_str = "01_03_ID_Mappings.sql"
    ...
    validate_and_execute(
        ...
        section=None,
        host="DATABASE_SETUP",
        **{"templates_dict": {"source_sql": rendered_template}},
    )
