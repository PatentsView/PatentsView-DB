...
INSERT INTO `PatentsView_{{ macros.ds_format(macros.ds_add(dag_run.data_interval_end | ds, -1), "%Y-%m-%d", "%Y%m%d") }}`.`government_interest` SELECT `patent_id`, `gi_statement` FROM `patent`.`government_interest` where version_indicator<='{{ dag_run.logical_date | ds_nodash }}';
...
INSERT INTO `PatentsView_{{ macros.ds_format(macros.ds_add(dag_run.data_interval_end | ds, -1), "%Y-%m-%d", "%Y%m%d") }}`.`patent_contractawardnumber` SELECT `patent_id`, `contract_award_number` FROM `patent`.`patent_contractawardnumber`  where version_indicator<='{{ dag_run.logical_date | ds_nodash }}';

INSERT INTO `PatentsView_{{ macros.ds_format(macros.ds_add(dag_run.data_interval_end | ds, -1), "%Y-%m-%d", "%Y%m%d") }}`.`patent_govintorg` SELECT `patent_id`, `organization_id` FROM `patent`.`patent_govintorg`  where version_indicator<='{{ dag_run.logical_date | ds_nodash }}';
