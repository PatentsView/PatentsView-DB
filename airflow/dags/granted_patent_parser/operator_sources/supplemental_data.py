...
def add_supplemental_operators(supplemental_dag, config, project_home, airflow_task_success,
    ...
    from updater.collect_supplemental_data.cpc_parser.cpc_class_parser import process_cpc_class_parser, \
        post_class_parser
    ...
    qc_cpc_current_wipo_operator = PythonOperator(task_id='qc_cpc_current_wipo', python_callable=post_cpc_wipo,
                                                  op_kwargs={'config': config}, dag=supplemental_dag,
                                                  on_success_callback=airflow_task_success,
                                                  on_failure_callback=airflow_task_failure)
    ...
    for prefix in prefixes:
        withdrawn_operator.set_upstream(prefix)
        download_cpc_operator.set_upstream(prefix)
    return [qc_cpc_current_wipo_operator, qc_withdrawn_operator]
