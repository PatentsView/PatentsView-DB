...
class GovtInterestTester(PatentDatabaseTester):
    def __init__(self, config):
        ...
        self.table_config = {'patent_contractawardnumber': {
            "fields": {"patent_id": {"data_type": "varchar", "null_allowed": False, "category": False},
                       "contract_award_number": {"data_type": "varchar", "null_allowed": False, "category": False}}},
            'patent_govintorg': {"fields": {
            ...
            'government_organization': {
                "fields": {"organization_id": {"data_type": "int", "null_allowed": False, "category": False},
                ...
                "related_entities": [
                    {"table": 'patent_govintorg', "source_id": 'organization_id', "destination": "organization_id"}]}}
    ...
    def generate_govt_int_samples(self):
        ...
        organization_wheres = ["go.`name` LIKE '%United States Government%'",
                               "go.`name` NOT LIKE '%United States Government%'"]
        where_combinations = itertools.product(date_where, organization_wheres)
        sampler_template = """
SELECT gi.patent_id,
       gi.gi_statement,
       go.organization_id, 
...
FROM   `government_interest` gi
       ...
       JOIN `patent_contractawardnumber` pca
         ON pca.patent_id = gi.patent_id
       JOIN `patent_govintorg` pg
         ON pg.patent_id = gi.patent_id
       JOIN government_organization go
...
LIMIT  5;        
        ...
        for where_combination in where_combinations:
            where_clause = "AND ".join(where_combination)
            ...
            with self.connection.cursor() as gov_int_cursor:
                ...
                for gov_int_row in gov_int_cursor:
                    self.qa_data['DataMonitor_govtinterestsampler'].append({"database_type": database_type,
                                                                            'update_version': version,
                                                                            'patent_id': gov_int_row[0],
                                                                            'gov_int_stmt': gov_int_row[1],
                                                                            'patent_contract_award_number': gov_int_row[
                                                                                4],
...
if __name__ == '__main__':
    qc = GovtInterestTester(get_config())
    qc.runTests()
