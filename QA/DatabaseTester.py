...
class DatabaseTester(ABC):
    def __init__(self, config, database_section, start_date, end_date, where_vi=True):
        ...
        self.qa_connection_string = get_connection_string(
            config, database="QA_DATABASE", connection="APP_DATABASE_SETUP"
        )
        ...
        if (
            self.class_called == "TextQuarterlyMergeTest"
            and database_section == "pgpubs_text"
        ):
        ...
        if (
            self.class_called == "TextQuarterlyMergeTest"
            and database_section == "patent_text"
        ):
    ...
    def find_previous_thursday(self):
        ...
        while (
            self.end_date.weekday() != 3
        ):  # 3 corresponds to Thursday (0 is Monday, 1 is Tuesday, and so on)
    ...
    def find_previous_tuesday(self):
        ...
        while (
            self.end_date.weekday() != 1
        ):  # 1 corresponds to Tuesday (0 is Monday, 2 is Wednesday, and so on)
            self.end_date -= timedelta(days=1)
    ...
    def query_runner(
        self, query, single_value_return=True, where_vi=False, vi_comparison="="
    ):
        ...
        if where_vi:
            ...
            if "where" and "main_table" in query:
                where_statement = (
                    f" and main_table.version_indicator {vi_comparison} '{vi_date}'"
                )
            elif "where" in query:
                where_statement = f" and version_indicator {vi_comparison} '{vi_date}'"
            else:
                where_statement = (
                    f" where version_indicator {vi_comparison} '{vi_date}'"
                )
        ...
        try:
            ...
            with self.connection as connection:
                with connection.cursor() as generic_cursor:
                    ...
                    query_end_time = time()
                    logger.info(
                        f"\t\tThis query took {query_end_time - query_start_time:.3f} seconds"
                    )
...
from {table_name}"""
        count_value = self.query_runner(
            query, single_value_return=True, where_vi=where_vi
        )
        if count_value < 1 and table_name not in [
            "rawuspc",
            "uspc",
    ...
    def test_blank_count(self, table, table_config, where_vi):
        for field in table_config["fields"]:
            if table_config["fields"][field]["data_type"] in [
                "varchar",
                "mediumtext",
                "text",
            ]:
...
where `{field}` = ''"""
                count_value = self.query_runner(
                    count_query, single_value_return=True, where_vi=where_vi
                )
                if count_value != 0:
                    exception_message = """
Blanks encountered in  table found:{database}.{table} column {col}. Count: {count}
                    """.format(
                        database=self.database_section,
                        table=table,
                        col=field,
                        count=count_value,
                    )
...
where INSTR(`{field}`, CHAR(0x00)) > 0"""
        count_value = self.query_runner(
            nul_byte_query, single_value_return=True, where_vi=where_vi
        )
        if count_value > 0:
            ...
            try:
                ...
                if self.connection.open:
                    self.connection.close()
            logger.info(
                f"attempted to correct newlines in {table}.{field}. re-performing newline detection query:"
            )
    ...
    def test_newlines(self, table, field, where_vi):
        ...
        if skip:
            logger.info(
                "newlines marked as permitted for this field. skipping newline test"
            )
        else:
            newline_query = f"""
            SELECT count(*) as count
            ...
            where INSTR(`{field}`, '\n') > 0"""
            count_value = self.query_runner(
                newline_query, single_value_return=True, where_vi=where_vi
            )
            if count_value > 0:
                logger.info(
                    f"{count_value} rows with unwanted newlines found in {field} of {table} for {self.database_section}. Correcting records ..."
                )
                makelogquery = (
                    f"CREATE TABLE IF NOT EXISTS `{table}_newline_log` LIKE {table}"
                )
...
group by 1"""
        count_value = self.query_runner(
            category_count_query, single_value_return=False, where_vi=False
        )
...
"""
        count_value = self.query_runner(
            zero_query, single_value_return=True, where_vi=where_vi
        )
        if count_value != 0:
            raise Exception(
    ...
    def check_for_indexes(self, table):
        if "webtool" not in table and table != "patent_lawyer_unique":
            ...
            if int(count_value) == 0:
                ...
                current_db = self.query_runner(
                    current_db_query, single_value_return=True
                )
                print(f"Connected to database: {current_db}")
                host_check_query = "SELECT @@hostname;"
                hostname = self.query_runner(host_check_query, single_value_return=True)
                print(f"Airflow is connected to MySQL host: {hostname}")
...
where name_first is not null and name_last is null"""
        count_value = self.query_runner(
            rawassignee_q, single_value_return=True, where_vi=where_vi
        )
        if count_value != 0:
            logger.info("THE FOLLOWING QUERY NEEDS ADDRESSING")
            logger.info(rawassignee_q)
            raise Exception(
                f"{self.database_section}.{table} Has Wrong Organization values"
            )

    def test_related_floating_entities(
        self, table_name, table_config, where_vi=False, vi_comparison="="
    ):
        if table_name not in self.exclusion_list and "related_entities" in table_config:
            for related_entity_config in table_config["related_entities"]:
                ...
                exists_table_count = self.query_runner(
                    exists_query, single_value_return=False, where_vi=False
                )
...
where main_table.{main_table_id} is null and related_table.{related_table_id} is not null
                    ...
                    related_count = self.query_runner(
                        related_query,
                    ...
                    if related_count > 0:
                        raise Exception(
                            ...
                            "main_table_id} in {main_table} for {db}".format(
                                ...
                                related_table=related_entity_config["related_table"],
                                main_table_id=related_entity_config["main_table_id"],
                                related_table_id=related_entity_config[
                                    "related_table_id"
                                ],
                                db=self.database_section,
                            )
            ...
            for related_entity_config in table_config["related_entities"]:
                ...
                if not exists_table_count:
                    ...
                    if (
                        ...
                        and related_entity_config["related_table"] == "assignee"
                    ) or (
                        table_name == "persistent_inventor_disambig"
                        and related_entity_config["related_table"] == "inventor"
                    ):
                    ...
                    additional_where = ""
                    if (
                        "custom_float_condition" in table_config
                        and table_config["custom_float_condition"] is not None
                    ):
                        additional_where = (
                            "and " + table_config["custom_float_condition"]
                        )
                    float_count_query = """
...
where related.{related_table_id} is null {additional_where}
                    ...
                    related_table_count = self.query_runner(
                        float_count_query, single_value_return=True, where_vi=False
                    )
...
group by 1               
                    """
            count_value = self.query_runner(
                count_query, single_value_return=False, where_vi=False
            )
            for count_row in count_value:
...
on t.country = cc.`alpha-2`
group by t.`{field}`"""
        row_count = self.query_runner(
            row_query, single_value_return=True, where_vi=False
        )
        count_value = self.query_runner(
            location_query, single_value_return=False, where_vi=False
        )
        for count_row in count_value:
    ...
    def save_qa_data(self):
        ...
        for qa_table in self.qa_data:
            ...
            if qa_table == "DataMonitor_topnentities":
            ...
            ]:  # table-specific QA tables that just identify records by update_version and db_type
                addl_condition = ""
                print_condition = ""
                ...
                raise NotImplementedError(
                    f"specification of existing rows to remove not implemented for {qa_table}.\ncolumns available: `{'`,`'.join(table_frame.columns)}`"
                )
            try:
                logger.info(
                    f"removing prior {qa_table} {self.database_type} records {print_condition}on {self.version}"
                )
                ...
                logger.info(
                    f"inserting new {qa_table} records for {self.version} and {self.database_type}"
                )
                table_frame.to_sql(
                    name=qa_table, if_exists="append", con=qa_engine, index=False
                )
    ...
    def load_text_length(self, table_name, field_name):
        text_length_query = (
            f"SELECT max(char_length(`{field_name}`)) " f"from `{table_name}`;"
        )
...
where invention_abstract is null """
    ...
    def runStandardTests(self):
        ...
        for table in self.table_config:
            ...
            logger.info(" -------------------------------------------------- ")
            self.test_blank_count(
                table, self.table_config[table], where_vi=self.where_vi
            )
            self.test_related_floating_entities(
                table, table_config=self.table_config[table], where_vi=self.where_vi
            )
            ...
            if (
                self.class_called != "ReportingDBTester"
                and "PostProcessingQC" not in self.class_called
            ):
                self.test_null_version_indicator(table)
            ...
            if "PostProcessingQC" not in self.class_called:
                self.test_related_floating_entities(
                    table_name=table,
                    table_config=self.table_config[table],
                    where_vi=(
                        True if self.class_called in vi_cutoff_classes else False
                    ),
                    vi_comparison=(
                        "<=" if self.class_called in vi_cutoff_classes else "="
                    ),
                )
                self.load_main_floating_entity_count(table, self.table_config[table])
            ...
            for field in self.table_config[table]["fields"]:
                ...
                if self.table_config[table]["fields"][field]["data_type"] in [
                    "mediumtext",
                    "longtext",
                    "text",
                ]:
                ...
                if (
                    self.table_config[table]["fields"][field]["data_type"] == "varchar"
                    and "id" not in field
                ):
...
if __name__ == "__main__":
    ...
    config = get_current_config(
        "granted_patent", **{"execution_date": datetime.date(2023, 10, 31)}
    )
    # fill with correct run_id
    run_id = "backfill__2020-12-29T00:00:00+00:00"
    pt = DatabaseTester(
        config,
        "PatentsView_20231231",
