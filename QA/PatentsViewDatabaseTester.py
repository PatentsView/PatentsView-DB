...
group by `{field}`
        ...
        if table_name not in self.exclusion_list:
            ...
            try:
                ...
                if table_name == self.central_entity:
                    count_query = "SELECT year(`date`) as `yr`, count(1) as `year_count` from {central_table} group by year(" \
                                  "`date`)".format(central_table=central_table)
                else:
    ...
    def load_prefix_counts(self, table_name):
        if table_name not in self.exclusion_list:
            if self.central_entity == 'patent':
                ...
                print("\tTesting entity counts by document_number type for {table_name}".format(table_name=table_name))
            try:
                if not self.connection.open:
                    self.connection.connect()
                central_table = self.central_entity if self.db_prefix is None else '{db}.{ce}'.format(
    ...
    def test_counts_by_location(self, table, field):
        """
        Test and collect count NULL values in different fields
        :param table:
        :param table_config:
